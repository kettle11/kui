        ...
        match element.element_type {
            ElementType::Row(spacing) => {
                ...
                self.tree.child_iter(node).fold((0., 0.), |s, n| {
                    ...
                    x += used_size.0 + spacing;
                    width -= used_size.0;
                    (s.0 + used_size.0 + spacing, s.1.max(used_size.1))
                })
            }
            ...
            ElementType::Column(spacing) => {
                ...
                self.tree.child_iter(node).fold((0., 0.), |s, n| {
                    ...
                    y += used_size.1;
                    height -= used_size.1;
                    (s.0.max(used_size.0), s.1 + used_size.1 + spacing)
                })
            }
            ...
            ElementType::CenterVertical => {
                ...
                for child in self.tree.child_iter(node) {
                    ...
                    self.render_element(text_properties, render_rect, child);
                }
                (element_width, element_height)
            }
            ElementType::TextSize(size) => {
                let text_properties = TextProperties { size };
                for child in self.tree.child_iter(node) {
                    self.render_element(&text_properties, rectangle, child);
                }
                (element_width, element_height)
